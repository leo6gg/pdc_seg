...
FILEPATH = '/tmp/pm/'
PDCDATAPATH = ""
if os.path.exists("/md/wmg/pdc/archive/"):
    PDCDATAPATH = "/md/wmg/pdc/archive/"
else:
    PDCDATAPATH = "/md/epdg/pdc/archive/"
DATABAKUPPATH = PDCDATAPATH + 'DataBakup/'
...
def process(FILEPATH, CSVPath):
        ...
        for valueItem in measValueList:
            ...
            if not temp[0].isdigit():
                ...
                if len(temp) == 4:
                    ...
                    #moduleName = 'G'+temp[2][-1]+'_'+temp[0]+'_'+temp[1]
                    measTypeListTemp.insert(1,'Instance')
                    
                #else:
                    #moduleName = 'G'+temp[2][-1]+'_'+temp[0]+'_'+temp[1]
            else:
                moduleName = 'G'+temp[0]+'_'+temp[1]+'_'+temp[2]
                if len(temp) == 4:
                    #moduleName = 'G'+temp[0]+'_'+temp[1]+'_'+temp[2]+'_'+temp[3]
                    #moduleName = 'G'+temp[0]+'_'+temp[1]+'_'+temp[2]
                    measTypeListTemp.insert(1,'Instance')
                #else:
                    #moduleName = 'G'+temp[0]+'_'+temp[1]+'_'+temp[2]
            ...
            measTypeDict = {moduleName:measTypeListTemp}
        ...
        for valueItem in valueList1:
            ...
            if not temp[0].isdigit():
                if len(temp) == 4:
                    #moduleName = 'G'+temp[2][-1]+'_'+temp[0]+'_'+temp[1]+'_'+temp[3]
                    moduleName = 'G'+temp[2][-1]+'_'+temp[0]+'_'+temp[1]
                    valueList.insert(1,temp[3].split('=')[1])
                else:
                    moduleName = 'G'+temp[2][-1]+'_'+temp[0]+'_'+temp[1]
            else:
                if len(temp) == 4:
    ...
    for groupItem in group:
        for item in measTypeDictList:
            tempList = []
            csvFileName = 'G'+groupItem+(list(item.keys())[0])[2:]
            csvFile = open(CSVPath+csvFileName+'.log','a+')
            writer = csv.writer(csvFile, delimiter='|', dialect='excel')
            #print ('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
            #print ('fields keys %s' % item.keys()[0])
            for item1 in valueDictList:
                #print ('fields keys in values %s' % item1.keys()[0])
                if csvFileName == (list(item1.keys())[0]):
                    tempList.append(list(item1.values())[0])
            tempList.insert(0,list(item.values())[0])
...
if __name__ == '__main__':
    ...
    #bakuppath = DATABAKUPPATH+time.strftime('%Y-%m-%d')
    bakuppath = DATABAKUPPATH
    if os.path.exists(bakuppath):
        process(FILEPATH,bakuppath + os.sep)
    else:
        bakuppath = mkdirs(bakuppath)
        process(FILEPATH,bakuppath + os.sep)
    ...
    for item in date:
        ...
        for file in os.listdir(targetDir):
            ...
            #print ('unique content = %s' % uniqueContent)
            for row in uniqueContent[1:]:
                timeStr = row.split('|')
                dateStr = timeStr[0].split(' ')
                contentDate = dateStr[0].replace('-','')
                #print ('date111111111 = %s' % dateStr)
                hour = dateStr[1].split(':')[0]
                min = dateStr[1].split(':')[1]
                #print 'date111111111 = %s' % date 
                if contentDate != item :
                    uniqueContent.remove(row)
                if contentDate == item and hour == '00' and min == '00':
                    ...
                    #print 'row ============== %s' % row
                    uniqueContent[uniqueContent.index(row)] = newDate + row[19:]
            ...
            if Call_Distribution[2] == 'Call' and Call_Distribution[3] == 'Distribution.log':
                rowName = uniqueContent[0]
                lastRow = uniqueContent[-1]
                uniqueContent = rowName + lastRow
                #print ('content[-1] = %s' % content)
            
            f.close()
            new_f = open(targetDir + os.sep + file, "w")
            new_f.writelines(uniqueContent)
            new_f.close()
    
    print ('Collect data successfully.')
    #delete the copied pm files
    if os.path.exists("/tmp/pm/"):
        os.system("rm -rf /tmp/pm")
